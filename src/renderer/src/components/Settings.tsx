...
function Settings({ settings }: SettingsProps) {
  ...
  const renderInput = {
    height: () => <Grid key={'height'} xs={4}><TextField sx={{ input: { color: 'white' }, label: { color: 'white' } }} label={'HEIGHT'} type={"Number"} value={activeSettings.height} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
      settingsChange('height', parseInt(event.target.value)) 
    }}/></Grid>,
    width: () => <Grid key={'width'} xs={4}><TextField sx={{ input: { color: 'white' }, label: { color: 'white' } }} label={'WIDTH'} type={"Number"} value={activeSettings.width} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
      settingsChange('width', parseInt(event.target.value))
    }}/></Grid>,
    dpi: () => <Grid key={'dpi'} xs={4}><TextField sx={{ input: { color: 'white' }, label: { color: 'white' } }} label={'DPI'} type={"Number"} value={activeSettings.dpi} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
      settingsChange('dpi', parseInt(event.target.value))
    }}/></Grid>,
    format: () => <Grid key={'format'} xs={4}><TextField sx={{ input: { color: 'white' }, label: { color: 'white' } }} label={'FORMAT'} type={"Number"} value={activeSettings.format} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
      settingsChange('format', parseInt(event.target.value))
    }}/></Grid>,
    fps: () => <Grid key={'fps'} xs={4}><TextField sx={{ input: { color: 'white' }, label: { color: 'white' } }} label={'FPS'} type={"Number"} value={activeSettings.fps} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
      settingsChange('fps', parseInt(event.target.value))
    }}/></Grid>,
    iBoxVersion: () => <Grid key={'iBoxVersion'} xs={4}><TextField sx={{ input: { color: 'white' }, label: { color: 'white' } }} label={'IBOX VERSION'} type={"Number"}inputProps={{ inputMode: 'numeric'}} value={activeSettings.iBoxVersion} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
      settingsChange('iBoxVersion', parseInt(event.target.value))
            }}/></Grid>,
    mediaDelay: () => <Grid key={'mediaDelay'} xs={4}><TextField sx={{ input: { color: 'white' }, label: { color: 'white' } }} label={'MEDIA DELAY'} type={"Number"} value={activeSettings.mediaDelay} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
      settingsChange('mediaDelay', parseInt(event.target.value))
              }}/></Grid>,
    phoneWorkMode: () => <Grid key={'phoneWorkMode'} xs={4}><TextField sx={{ input: { color: 'white' }, label: { color: 'white' } }} label={'PHONE WORK MODE'} type={"Number"} value={activeSettings.phoneWorkMode} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
